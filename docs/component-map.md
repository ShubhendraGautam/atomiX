...
| Accelerator role | `role` | `role.none` shell default; `role.loopback` doorbell/descriptor contract proof; `role.tpu-lite` (folded int8 GEMM) and `role.gpu-compute` (8-lane SIMT vector engine) real accelerators |
...
- Accelerator roles are `role` components behind the fixed shell window at
  ...
  (folded int8 GEMM) and `role.gpu-compute` (SIMT vector engine) are the real
