// muldiv.fast-mul: RV32M execution unit with a single-cycle multiplier and
// the reference 32-cycle restoring divider.  One signed 33x33 product covers
// MUL/MULH/MULHSU/MULHU because each operand is sign- or zero-extended by one
// bit according to its own signedness; synthesis maps the multiply onto FPGA
// DSP blocks (MULT18X18D on the ECP5).  The start/busy/done contract matches
// muldiv.iterative32 — operands are sampled at the start edge, done lasts one
// cycle, the result holds during done — but a multiply completes on the first
// busy cycle instead of the thirty-second.
module muldiv (
  input  logic        clk,
  input  logic        rst,
  input  logic        start,
  ...
  assign busy = busy_q;
  assign done = busy_q && (is_mul_q || count_q == 5'd31);
  always_comb begin
    if (is_mul_q) begin
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      if (is_mul_q || count_q == 5'd31) busy_q <= 1'b0;
    end
  end

  // verilator lint_off UNUSED
  wire unused_mul = ^{mul_full[65:64]};
